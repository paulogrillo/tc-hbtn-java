...
public class Consulta {
    ...
    public static List<Pedido> obterPedidosComEletronicos(List<Pedido> pedidos) {
      	return pedidos.stream().filter(pedido -> pedido.getProdutos()
                      .stream().anyMatch(p -> p.getCategoria()
    ...
    public static List<Produto> aplicar15PorcentoDescontoEletronicos(List<Produto> produtos) {
        produtos.forEach(produto -> {
            if (produto.getCategoria().equals(CategoriaProduto.ELETRONICO)){
                Double valorDesconto = produto.getPreco() - (produto.getPreco()*0.15);
                produto.setPreco(valorDesconto);
            }
        });
        return produtos;
    }
